...
// Events the kernel emits. Plugins may also define their own event names via
// api.on/emit — this list is documentation, not an allowlist.
...
function createHost(options) {
  ...
  function scanEntry(layer, entry) {
    ...
    if (manifest) {
      ...
      rec.description = typeof manifest.description === 'string' ? manifest.description : null;
    ...
    // Themes join the global registry only if the record was accepted —
    // otherwise a plugin skipped for a name collision would leak its themes.
    if (addRecord(rec)) {
      for (const def of rec.themes) {
  ...
  function themeCss(id) {
    ...
    if (!def) return null;
    // Cache, but re-read when the file changes on disk (dev-friendly).
    let mtimeMs = 0;
    try { mtimeMs = fs.statSync(def.cssPath).mtimeMs; } catch (e) { /* deleted — fall through to read error */ }
    const cached = cssCache.get(id);
    if (cached && cached.mtimeMs === mtimeMs) return cached.css;
    try {
      const css = fs.readFileSync(def.cssPath, 'utf8');
      cssCache.set(id, { css, mtimeMs });
      return css;
    } catch (e) {
  ...
  return {
    ...
    shutdown,
    _internal: { plugins, commands, services, hooks, trayItems, themes },
  };
}
